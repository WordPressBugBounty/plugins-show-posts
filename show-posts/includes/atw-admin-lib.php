...
{
    ?>
    &nbsp;&nbsp;&larr;&nbsp;<a style='text-decoration:none;'
                               href="javascript:atw_media_lib('<?php echo esc_html($fillin); ?>');"><img
            src="<?php echo esc_url(atw_posts_plugins_url('/images/media-button.png')); ?>"
...
function atw_posts_download_link($desc, $filebase, $ext, $time)
{
    ...
    <?php esc_html_e('Download', 'show-posts' /*adm*/);
    echo '</span></a> - ';
    echo wp_kses_post($desc);
    echo ' &nbsp;';

    echo 'Save as:';
    echo ' ' . esc_html($filename) . "<br /><br />\n";
}
...
// phpcs:ignore WordPress.Security.NonceVerification.Missing -- verified elsewhere
    ...
    // phpcs:ignore WordPress.Security.NonceVerification.Missing -- file upload handled in admin flow
        $filename = $_FILES['post_uploaded']['tmp_name'];
        //$filename = sanitize_file_name(wp_unslash($_FILES['post_uploaded']['tmp_name']));
    } else
        $filename = "";
...
function atw_posts_filter_textarea($id, $desc, $br = '<br />') {
    ...
    <textarea style="margin-bottom:-8px;" cols=32 rows=1 maxlength=64
              name="<?php echo esc_html($id); ?>"><?php echo wp_kses_post(sanitize_text_field(atw_posts_get_filter_opt($id))); ?></textarea>
    &nbsp;
...
function atw_posts_filter_val($id, $desc, $br = '<br />')
{
    ...
    <input class="regular-text" type="text" style="width:50px;height:22px;" name="<?php echo esc_html($id); ?>"
           value="<?php echo wp_kses_post(sanitize_text_field(atw_posts_get_filter_opt($id))); ?>"/>
    &nbsp;
    <?php echo wp_kses_post($desc . '</label></div>' . $br) . "\n";
}
